...
bool BinSearch(int n,int key)//顺序检索函数 
{
	sort(l,l+n);//生成有序序列 
	int i=0;count0=0;bool flag=false;
	//精确度不够t_s=clock();
...
void test(ifstream &in,ofstream &out,int N)//测试函数 
{
	...
	for(i=0;i<100;i++)
	{
		bool f=BinSearch(N,a0[i]);
		min_=min(min_,dft*1000);
		max_=max(max_,dft*1000);
		ave_+=dft*10;
		if(f) 
		{
			cout<<str3<<a0[i]<<"\t"<<str1<<count0<<str2<<dft*1000<<" ms\n"; 
			out<<str3<<a0[i]<<"\t"<<str1<<count0<<str2<<dft*1000<<" ms\n";
		}
		else 
		{
			cout<<str4<<a0[i]<<"\t"<<str1<<count0<<str2<<dft*1000<<" ms\n";
			out<<str4<<a0[i]<<"\t"<<str1<<count0<<str2<<dft*1000<<" ms\n";
		} 
	}
	out<<"最小查找时间:   "<<min_<<" ms\t"<<"最大查找时间：  "<<max_<<" ms\t"<<"平均查找时间：  "<<ave_<<" ms"; 
	out<<"\n";
	min_=100;max_=0;ave_=0;
	out<<"100次失败查找\n";
	//100次失败查询  
	for(i=0;i<100;i++)
	{
		bool f=BinSearch(N,b0[i]);
		min_=min(min_,dft*1000);
		max_=max(max_,dft*1000);
		ave_+=dft*10;
		if(f) 
		{
			cout<<str3<<b0[i]<<"\t"<<str1<<count0<<str2<<dft*1000<<" ms\n";
			out<<str3<<b0[i]<<"\t"<<str1<<count0<<str2<<dft*1000<<" ms\n";
		}
		else 
		{
			cout<<str4<<b0[i]<<"\t"<<str1<<count0<<str2<<dft*1000<<" ms\n";
			out<<str4<<b0[i]<<"\t"<<str1<<count0<<str2<<dft*1000<<" ms\n";
		}
	}
	out<<"最小查找时间:   "<<min_<<" ms\t"<<"最大查找时间：  "<<max_<<" ms\t"<<"平均查找时间：  "<<ave_<<" ms"; 
	out<<"\n";
